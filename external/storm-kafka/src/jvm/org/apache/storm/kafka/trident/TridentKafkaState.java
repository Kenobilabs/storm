...
public class TridentKafkaState implements State {
  ...
  public void withStatsD(Map conf, TopologyContext context) {

    // assign specific TaskId
    baseMetricName = Util.clean(context.getTaskData(KAFKACONSTANTS.STATSD) + "."
        + Util.getLocalIpAddress() + "." + context.getThisWorkerPort());

    // statsd Prepare
    if (stormConf != null && stormConf.containsKey(KAFKACONSTANTS.STATSD)
        && "YES".compareToIgnoreCase(stormConf.get(KAFKACONSTANTS.STATSD).toString()) == 0) {

      statsdConsumer = new StatsdMetricConsumer();
      statsdConsumer.prepare(null, stormConf, null, null);
  ...
  public void updateStateWithoutStatsD(List<TridentTuple> tuples, TridentCollector collector) {
    ...
    try {
      ...
      if (exceptions.size() > 0) {
        String errorMsg = "Could not retrieve result for messages " + tuples + " from topic = "
            + topic + " because of the following exceptions: \n";
  ...
  public void updateStateWithStatsD(List<TridentTuple> tuples, TridentCollector collector) {
    String topic = null;
    try {
      statsdConsumer.incrementCounter(baseMetricName + KAFKACONSTANTS.ATTEMPT);
      List<Future<RecordMetadata>> futures = new ArrayList<>(tuples.size());
      for (TridentTuple tuple : tuples) {
        topic = topicSelector.getTopic(tuple);
